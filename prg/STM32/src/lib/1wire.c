...
k_uchar onewire_dev_search(onewire_handler_s *onewire_handler, k_uchar *dev_id)
	{
	int id_bit_number;
	...
	if (!LastDeviceFlag)
		...
		// 1-Wire reset
		if (onewire_bus_reset(onewire_handler) != 0x00)
			{
			// reset the search
			LastDiscrepancy = 0;
		...
		onewire_write_byte(onewire_handler, ONE_WIRE_CMD_SEARCH_ROM);

		  // loop to do the search
		do
			{
			// read a bit and its complement
			id_bit = onewire_read_bit(onewire_handler);
			cmp_id_bit = onewire_read_bit(onewire_handler);
			...
			if ((id_bit == 1) && (cmp_id_bit == 1))
				...
				if (id_bit != cmp_id_bit)
					...
					if (id_bit_number < LastDiscrepancy)
						search_direction = ((dev_id[rom_byte_number] & rom_byte_mask) > 0);
					else
				...
				if (search_direction == 1)
					dev_id[rom_byte_number] |= rom_byte_mask;
				else
					dev_id[rom_byte_number] &= ~rom_byte_mask;

				// serial number search direction write bit
				onewire_write_bit(onewire_handler, search_direction);
				...
				if (rom_byte_mask == 0)
					{
					onewire_crc8(&crc8, dev_id[rom_byte_number]); // accumulate the CRC
					rom_byte_number++;
					rom_byte_mask = 1;
					}
				}
...
k_uchar onewire_dev_search_first(onewire_handler_s *onewire_handler, k_uchar *dev_id, k_uchar dev_family)
	...
	return onewire_dev_search(onewire_handler, dev_id);
	}
